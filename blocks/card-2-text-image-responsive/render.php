...
<div <?= $wrapper_attributes; ?>>
  ...
  <?php if (!empty($decor)):
    foreach ($decor as $d):
      ...
      <img class="card-2-text-img-resp__decor <?= esc_attr($side_class); ?>" src="<?= $url; ?>" alt="<?= $alt; ?>"
        loading="lazy" decoding="async" />
    <?php endforeach; endif; ?>

  <div class="card-2-text-img-resp__inner">

    <div class="card-2-text-img-resp__col card-2-text-img-resp__text" style="--panel-bg: <?= esc_attr($textBg ?: 'transparent'); ?>;
      ...
      <div class="<?= esc_attr($panel_classes); ?>">
        ...
        <?php if (!empty($textDecor)):
          ...
          foreach ($textDecor as $td):
            ...
            // check for specific image filename to apply special width class
            $is_special = (strpos($turl, 'Packing_Tips_for_Stress-Free_Travel') !== false);
            $special_class = $is_special ? ' is-special-width' : '';
            ?>
            <img
              class="card-2-text-img-resp__panel-decor card-2-text-img-resp__panel-decor--<?= esc_attr($pos); ?><?= $decor_anim_class ? ' ' . esc_attr($decor_anim_class) : ''; ?><?= $special_class; ?>"
          ...
          <div class="<?= esc_attr($headline_classes_wrap); ?>">
            <?php if ($has_icon_left): ?>
              <span class="card-2-text-img-resp__headline-icon">
                <img src="<?= esc_url($headingIconUrl); ?>" alt="<?= esc_attr($headingIconAlt); ?>" loading="lazy"
                  decoding="async" />
              </span>
            <?php endif; ?>
    ...
    <div class="card-2-text-img-resp__col card-2-text-img-resp__media">
      <?php if ($imageUrl): ?>
        <figure class="card-2-text-img-resp__figure" style="--img-radius: <?= esc_attr($imgRadius); ?>px;">
          <img class="card-2-text-img-resp__image" src="<?= esc_url($imageUrl); ?>" alt="<?= esc_attr($imageAlt); ?>"
            loading="lazy" decoding="async" />
        </figure>
      <?php endif; ?>
    </div>
