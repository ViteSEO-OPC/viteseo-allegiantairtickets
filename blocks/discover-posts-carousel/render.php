...
$section_description = isset($attributes['sectionDescription']) ? sanitize_text_field($attributes['sectionDescription']) : 'test';
...
$print_card = function (array $card) use ($mode, $metrics_cutoff) {
	...
	?>
	<article class="discover-card" data-post-id="<?php echo esc_attr($postId); ?>"
		data-liked="<?php echo $liked ? '1' : '0'; ?>" data-likes="<?php echo esc_attr($likes); ?>">
		<div class="discover-card__like-wrap">
			<button type="button" class="discover-card__like-btn" aria-pressed="<?php echo $liked ? 'true' : 'false'; ?>"
				<?php if (!$postId)
					echo 'disabled'; ?>>
		...
		<a href="<?php echo esc_url($url); ?>" class="discover-card__link">
			<div class="discover-card__thumb">
				<?php if ($thumb): ?>
					<img src="<?php echo esc_url($thumb); ?>" alt="<?php echo esc_attr($alt); ?>" class="discover-card__img" />
				<?php else: ?>
...
<section class="carousel-block discover-carousel child-block">
	<div class="discover-carousel__header">
		<div>
			...
			<p class="discover-carousel__intro">
	...
	<?php if ($isCarousel): ?>
		...
		<div id="<?php echo esc_attr($scope_id); ?>" class="peek-slider is-mobile" role="region"
			aria-roledescription="carousel" aria-label="<?php echo $ariaLabel_esc; ?>">
		...
		<div id="<?php echo esc_attr($id); ?>" class="carousel slide generic-carousel is-desktop" role="region"
			aria-roledescription="carousel" aria-label="<?php echo $ariaLabel_esc; ?>"
			data-cols-xs="<?php echo (int) ($cols['xs'] ?? 1); ?>" data-cols-sm="<?php echo (int) ($cols['sm'] ?? 2); ?>"
			data-cols-md="<?php echo (int) ($cols['md'] ?? 3); ?>" data-cols-lg="<?php echo (int) ($cols['lg'] ?? 4); ?>"
			data-cols-xl="<?php echo (int) ($cols['xl'] ?? 5); ?>" <?php echo $carouselDataAttr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<?php if ($showIndicators && count($slides) > 1): ?>
				<div class="carousel-indicators">
					<?php foreach ($slides as $i => $_): ?>
						<button type="button" data-bs-target="#<?php echo esc_attr($id); ?>"
							data-bs-slide-to="<?php echo esc_attr($i); ?>" <?php echo 0 === $i ? 'class="active" aria-current="true"' : ''; ?> aria-label="<?php printf('Slide %d', $i + 1); ?>"></button>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
			...
			<?php if ($showControls && count($slides) > 1): ?>
				<button class="carousel-control-prev" type="button" data-bs-target="#<?php echo esc_attr($id); ?>"
					data-bs-slide="prev" aria-label="Previous">
					<span class="slider-prev" aria-hidden="true">&#10094;</span>
				</button>
				<button class="carousel-control-next" type="button" data-bs-target="#<?php echo esc_attr($id); ?>"
